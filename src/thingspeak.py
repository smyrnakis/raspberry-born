...
def writeDataFile(humidity, temperature):
    """Writting last measured values in files."""
...
def validateDHT(humidity, temperature):
    ...
    while ( abs(previousHumidity - humidity) > 5 or abs(previousTemperature - temperature) > 5 ) and whileCounter < 4 \
        ...
        if all_args.d: print("[THINKGSPEAK] RE-MEASUREMENT T:",temperature,"H:",humidity,"whileCounter:",whileCounter)
...
def measure_send():
    ...
    if all_args.d: print("[THINKGSPEAK] Average-1:",load1, "Average-5:",load5, "Average-15:",load15)
    ...
    try:
        ...
        if previousHumidity == 100 or previousTemperature == 100 :
            if all_args.d: print("[THINKGSPEAK] initialising 'previous' values")
            previousHumidity = humRoom
            previousTemperature = tempRoom

        humidityRoom, temperatureRoom = validateDHT(humRoom, tempRoom)
        if all_args.d: print("[THINKGSPEAK] >>>> VALIDATED measurement T:",temperatureRoom, "H:",humidityRoom)
    ...
    try:
        ...
        if all_args.d: print("[THINKGSPEAK] connection to Thingspeak:", response.status, response.reason)
        # data = response.read()
        conn.close()
    except:
